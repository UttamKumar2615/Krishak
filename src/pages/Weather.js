...
class WeekContainer extends React.Component {
  ...
  render() {
    const onSearch = (city) => {
      console.log(city);
      const weatherURL = `http://api.openweathermap.org/data/2.5/forecast?q=city&units=imperial&APPID=a08ae86978a14276276c5e99283277b6`;
      fetch(weatherURL)
        .then((res) => res.json())
        .then((data) => {
    ...
    return (
      <div className="container">
        <Row style={{ margin: 15 }}>
          ...
          <Col md={2}>
            <Button color="primary" onClick={() => onSearch(x)}>
              {" "}
              Check Weather
            </Button>
          </Col>
        ...
        <Row className="center">
          <h3>Recommending Crops for {x}</h3>
        </Row>
        <br />
        <div className="row justify-content-center">{this.Recommend(x)}</div>
