...
class Cart extends React.Component {
  ...
  componentDidMount() {
    ...
    firebase.auth().onAuthStateChanged((user) => {
      if (user) {
            ...
            if (snapshot.exists()) {
              ...
              for (let u of userdb) {
                if (u.category === "farmer" && u.email === user.email) {
                  alert(
                    "Dear user, Farmers can't have a cart, please head to the home page"
                  );
        ...
        alert(
          "Oops! Seems like you're logged out...\nPlease login to continue."
        );
        this.props.history.push("/home");
      }
    });
  ...
  render() {
    ...
    if (!this.state.isLoading && this.state.products.length) {
      cartContent = this.state.products.map((product, pos) => {
        return (
          <div id="info" key={pos}>
            <hr />
            <h2 id="highlight">{product.crop}</h2>
            <p>
              Sold By {product.farmerName} at &#8377;{product.price}/Kg
            </p>
      ...
      cartContent = (
        <div id="info" className="content">
          ...
          <p>
            Would you like to{" "}
            <Link to="/home" style={{ color: "#49A078" }}>
              search
            </Link>{" "}
            for groceries?
          </p>
    ...
    return (
      <div>
        <h1 id="highlight" className="content">
          My Cart
        </h1>
        {cartContent}
      </div>
